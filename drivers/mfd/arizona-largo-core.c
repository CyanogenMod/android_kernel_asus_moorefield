...
#include <linux/mfd/arizona/core-largo.h>
#include <linux/mfd/arizona/registers-largo.h>

#include "arizona-largo.h"
...
static const char * const cs47l24_core_supplies[] = {
	"AVDD",
	"DBVDD",
};
...
static int arizona_runtime_suspend(struct device *dev)
{
	...
	if (arizona->external_dcvdd) {
		...
		case WM1831:
			...
			break;
				/**
				 * * ISOLATION_CONTROL reg is not available CS47L24
				 * * */
		case CS47L24:
			break;
		default:
...
int arizona_dev_init(struct arizona *arizona)
{
	...
	case CS47L35:
		...
		dev_err(dev, "arizona device init num_core =%d\n", arizona->num_core_supplies);
		break;
	case CS47L24:
		for (i = 0; i < ARRAY_SIZE(cs47l24_core_supplies); i++)
			arizona->core_supplies[i].supply
				= cs47l24_core_supplies[i];
		arizona->num_core_supplies = ARRAY_SIZE(cs47l24_core_supplies);
		dev_err(dev, "arizona device init num_core =%d\n", arizona->num_core_supplies);
		break;
	default:
	...
	if (ret != 0) {
		dev_err(dev, "guoqing: Failed to request core supplies: %d\n",
			ret);
		goto err_early;
	}
	...
	dev_err(dev, "Get device id from codec\n");
	...
	dev_err(dev, "Device id = 0x%x , ret=%d\n", reg, ret);
	...
	for (i = 0; i < ARIZONA_MAX_MICBIAS; i++) {
		...
		dev_err(dev, "micbias%d mV = %d\n", i, arizona->pdata.micbias[i].mV);
	...
	case WM1831:
	case CS47L24:
		pr_err("phoenix largo device create\n");
	...
	dev_err(dev, "Arizona core dev init with err = %d !\n", ret);
