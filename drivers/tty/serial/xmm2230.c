...
static void ifx_spi_timeout(unsigned long arg)
{
	...
	int tp1;
	spin_lock_irqsave(&ifx_dev->timer_lock, flags);
	tp1 = gpio_get_value(ifx_dev->gpio.srdy);
	ifx_dev->spi_ipc_timeout_count++;

	if (ifx_dev->spi_ipc_timeout_count == IFX_SPI_IPC_TIMEOUT_ATTEMPTS) {
		/* set HANGUP here, but clear in activate API to avoid unexpected srdy*/
		set_bit(IFX_SPI_STATE_IPC_HANGUP, &ifx_dev->flags);
		spin_unlock_irqrestore(&ifx_dev->timer_lock, flags);
		mrdy_set_low(ifx_dev);
		dev_warn(&ifx_dev->spi_dev->dev, "*** SPI Timeout, modem hangup ***");
		ifx_dev->spi_ipc_timeout_count = 0;
		tty_port_tty_hangup(&ifx_dev->tty_port, false);
		clear_bit(IFX_SPI_STATE_TIMER_PENDING, &ifx_dev->flags);
		ifx_spi_power_state_clear(ifx_dev,
						  IFX_SPI_POWER_DATA_PENDING);
	} else {
		int tp2 = gpio_get_value(ifx_dev->gpio.srdy);
		spin_unlock_irqrestore(&ifx_dev->timer_lock, flags);
		if (tp2 > tp1) {
		/* handle a very rare case - srdy interrupt comes during irq spin lock*/
			ifx_dev->spi_ipc_timeout_count = 0;
			dev_warn(&ifx_dev->spi_dev->dev, "*** SPI Timeout, "
					"but srdy recovered during irq spin lock ***");
			if (test_bit(IFX_SPI_STATE_IO_AVAILABLE, &ifx_dev->flags)) {
				...
				ifx_spi_power_state_clear(ifx_dev,
								IFX_SPI_POWER_DATA_PENDING);
				dev_err(&ifx_dev->spi_dev->dev, "*** fatal ERROR, "
						"unable to handle it, SPI port need to be activated ***");
			}
			return;
		}
		if (test_bit(IFX_SPI_STATE_TIMER_PENDING, &ifx_dev->flags)) {
			/* have to assert mrdy due to no srdy found finally*/
			...
			ifx_dev->spi_ipc_timeout_count = 0;
			dev_warn(&ifx_dev->spi_dev->dev, "*** SPI Timeout, but srdy recovered "
						"immediately at the entry of timeout handler ***");
		}
	}
}
...
static void ifx_spi_handle_srdy(struct ifx_spi_device *ifx_dev)
{
	...
	if (test_bit(IFX_SPI_STATE_IPC_RECOVER, &ifx_dev->flags)) {
		ifx_dev->spi_ipc_timeout_count = 0;
		clear_bit(IFX_SPI_STATE_IPC_RECOVER, &ifx_dev->flags);
		dev_warn(&ifx_dev->spi_dev->dev, "SPI Timeout, srdy did come, but late and ignored\n");
	}
...
static irqreturn_t ifx_spi_srdy_interrupt(int irq, void *dev)
{
	...
	if (test_bit(IFX_SPI_STATE_IPC_HANGUP, &ifx_dev->flags)) {
		dev_warn(&ifx_dev->spi_dev->dev, "SPI Timeout, srdy did come, but too late and hangup reported\n");
		return IRQ_HANDLED;
	}
...
static int ifx_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	clear_bit(IFX_SPI_STATE_IPC_HANGUP, &ifx_dev->flags);
	clear_bit(IFX_SPI_STATE_IPC_RECOVER, &ifx_dev->flags);
...
static int ifx_spi_pm_resume(struct device *dev)
{
	struct spi_device *spi = to_spi_device(dev);
	struct ifx_spi_device *ifx_dev = spi_get_drvdata(spi);
	printk(KERN_INFO "enter ifx_spi_pm_resume\n");
	tasklet_enable(&ifx_dev->io_work_tasklet);
	printk(KERN_INFO "exit ifx_spi_pm_resume\n");
	return 0;
}
