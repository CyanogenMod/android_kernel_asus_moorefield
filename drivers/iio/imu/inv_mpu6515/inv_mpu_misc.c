...
//<asus_zx20150328>+>>
#if 1 //for Intel I2C multiple blocks issue
...
//printk("zxtest1 mpu_memory_write bank res %d\n", res);
	msgs[0].addr = mpu_addr;
	msgs[0].flags = 0;
	msgs[0].buf = addr;
...
//printk("zxtest1 mpu_memory_write addr res %d\n", res);
	msgs[0].addr = mpu_addr;
	msgs[0].flags = 0;
	msgs[0].buf = (u8 *)buf;
	msgs[0].len = len + 1;
...
//	printk("zxtest1 mpu_memory_write buf res %d\n", res);
	INV_I2C_INC_MPUWRITE(3 + 3 + (2 + len));
#ifdef CONFIG_DYNAMIC_DEBUG
	{
		char *write = 0;
		pr_debug("%s zxtest WM%02X%02X%02X%s%s - %d\n", st->hw->name,
		    ...
		    u32 len, u8 *data)
{
	...
	if (res != 1) {
		...
		res = 0;
//printk("zxtest1 mpu_memory_read buf res %d\n", res);
	INV_I2C_INC_MPUWRITE(3 + 3 + 3);
	INV_I2C_INC_MPUREAD(len);
#ifdef CONFIG_DYNAMIC_DEBUG
	{
		char *read = 0;
		pr_debug("%s zxtest2 RM%02X%02X%02X%02X - %s%s\n", st->hw->name,
...
int inv_set_step_buffer_time(struct inv_mpu_state *st, u16 value)
{
    ...
    // Pedometer executes at 50Hz so unit is 20ms
...
int inv_set_step_peak_threshold(struct inv_mpu_state *st, u8 value)
{
    ...
    peak_threshold=(u32)value*(5368709);//(2^29*threshold(G)/100)
...
//			DMP_IMAGE_SIZE, size);
	...
	//<asus_zx20150328>+>>
	if (result) printk("mpu6515 inv_verify_firmware  return = %d\n",
			result);
		//goto firmware_write_fail;
	//<asus_zx20150328>+<<
...
//printk("zxtest inv_dmp_firmware_read \n");
