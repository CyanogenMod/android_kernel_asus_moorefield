...
#define VENDOR  "mpu6515"
struct proc_dir_entry *mpusensor_entry=NULL;
struct proc_dir_entry *gyroscopempusensor_entry=NULL;
...
static int inv_write_accel_fs(struct inv_mpu_state *st, int fs)
{
	...
	if (fs == st->chip_config.accel_fs)
		return 0;
	if (INV_MPU3050 == st->chip_type)
		result = st->slave_accel->set_sfs(st, fs);
	else
		result = inv_i2c_single_write(st, reg->accel_config,
	...
	struct device_attribute *attr, char *buf)
{
	...
	scale_t = cur_off +
		inv_q30_mult((int)temp << MPU_TEMP_SHIFT, (int)cur_scale);

	INV_I2C_INC_TEMPREAD(1);

	return sprintf(buf, "%lld%lld\n", (s64)scale_t, get_time_ns());
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!!data) {
		result = st->set_power_state(st, true);
		if (result) {
			printk("wxdebug : inv_master_enable_store  :set_power_state true failed!!!"); //<asus-wx20150611> for debug rotation error
			goto end_enable;
		}
	}
	result = set_inv_enable(indio_dev, !!data);
	if (result) {
		printk("wxdebug : inv_master_enable_store  :set_inv_enable %d failed!!!", data); //<asus-wx20150611> for debug rotation error
		goto end_enable;
	}
	if (!data) {
		result = st->set_power_state(st, false);
		if (result) {
			printk("wxdebug : inv_master_enable_store  :set_power_state false failed!!!"); //<asus-wx20150611> for debug rotation error
			goto end_enable;
		}
	}
	result = count;
...
static ssize_t get_mpu65xx_gyro_rawdata(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	short rawdata_x=0,rawdata_y=0,rawdata_z=0;
        ...
        result = inv_i2c_read(st, REG_PWR_MGMT_1, 1,&pwr1);
        result = inv_i2c_read(st, REG_PWR_MGMT_2, 1,&pwr2);
        printk("wlin:get gyro raw pw1:%x, pw2:%x\n",pwr1,pwr2);

        result = inv_i2c_single_write(st, REG_PWR_MGMT_1, 0);
	result = inv_i2c_single_write(st, REG_PWR_MGMT_2, 0);
	msleep(100);
	result = inv_i2c_read(st, REG_RAW_GYRO, 6, buffer);
	...
	result = inv_i2c_single_write(st, REG_PWR_MGMT_2, pwr2);
        result = inv_i2c_single_write(st, REG_PWR_MGMT_1, pwr1);
...
static ssize_t get_mpu65xx_gsensor_rawdata(struct device *dev, struct device_attribute *devattr, char *buf)
{
  ...
  struct inv_mpu_state *st = iio_priv(dev_get_drvdata(dev));
	...
	u8 pwr1,pwr2;
	short rawdata_x=0,rawdata_y=0,rawdata_z=0;
	//<ASUS-danielchan20150514>>>>>>>>>+
	short report_rawdata_x=0,report_rawdata_y=0,report_rawdata_z=0;
	s8 *m;
	int i=0;
	//<ASUS-danielchan20150514><<<<<<<<+
        int result;
        ...
        result = inv_i2c_read(st, REG_PWR_MGMT_1, 1,&pwr1);
        result = inv_i2c_read(st, REG_PWR_MGMT_2, 1, &pwr2);
        printk("wlin:get accel raw pw1:%x, pw2:%x\n",pwr1,pwr2);

	result = inv_i2c_single_write(st, REG_PWR_MGMT_1, 0);
	result = inv_i2c_single_write(st, REG_PWR_MGMT_2, 0);
        msleep(100);
	result = inv_i2c_read(st, REG_RAW_ACCEL, 6, buffer);
	...
	for(i=0;i<9;i=i+3) {
		if(m[i]!=0) {
		report_rawdata_x=m[i]*rawdata_x;
		}
		if(m[i+1]!=0) {
		report_rawdata_y=m[i+1]*rawdata_y;
		}
		if(m[i+2]!=0) {
		report_rawdata_z=m[i+2]*rawdata_z;
		}
	}
	return sprintf(buf, "%d , %d , %d\n",report_rawdata_x, report_rawdata_y, report_rawdata_z);
	//return sprintf(buf, "%d , %d , %d\n",-rawdata_x, rawdata_y, -rawdata_z);
	//<ASUS-danielchan20150514><<<<<<<<+
}
#endif
...
//<asus_zx20150328>+>>
#define COMPASS_ON_PRIMARY_I2C 1
//MPU-6515 + AKM09911 on the secondary I2C interface
//gyro_platform_data_for_intel_platform
...
//<asus-danielchan20150402>>>>>>>>>+
static struct mpu_platform_data ZD550KL_gyro_platform_data = {
        .int_config  = 46,
        .level_shifter = 0,
        .orientation = {  0,  -1,  0,
                          1,  0,   0,
...
//<asus-olaf20150413+>
static struct mpu_platform_data ZE600KL_gyro_platform_data = {
        .int_config  = 0x10,
        .level_shifter = 0,
        .orientation = {  0,  1,  0,
                          1,  0,  0,
...
//<asus-annacheng20150129>>>>>>>>>>>>>>>>>>>>>>>+
static struct inv_mpu_state  *inv_mpu;
static int mpusensor_proc_show(struct seq_file *m, void *v) {

		int ret = 0,retval=0;
		u8 did;

		struct inv_mpu_state  *st = inv_mpu ;
		//printk("anna-sensor idReg : 333 \n");
		retval = inv_i2c_read(st, REG_WHOAMI, 1, &did);
		//printk("anna-sensor idReg : 444 \n");
		//retval = i2c_smbus_read_byte_data(kx022->client, WHO_AM_I);
		if (retval < 0)
			printk(KERN_INFO "[mpu6515] i2c err\n");
		...
		if((did== MPU6500_ID) || (did== MPU6515_ID)){//who am i
...
static int mpusensor_proc_open(struct inode *inode, struct  file *file) {
  return single_open(file, mpusensor_proc_show, NULL);
}

static const struct file_operations mpusensor_proc_fops = {
  .owner = THIS_MODULE,
  .open = mpusensor_proc_open,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

int create_asusproc_mpusensor_status_entry(void){
	mpusensor_entry = proc_create("gsensor_status", S_IWUGO| S_IRUGO, NULL,&mpusensor_proc_fops);
	if (!mpusensor_entry)
		 return -ENOMEM;
...
static int  gyroscopempusensor_proc_show(struct seq_file *m, void *v) {
...
static int gyroscopempusensor_proc_open(struct inode *inode, struct  file *file) {
  return single_open(file, gyroscopempusensor_proc_show, NULL);
}

static const struct file_operations gyroscopempusensor_proc_fops = {
  .owner = THIS_MODULE,
  .open = gyroscopempusensor_proc_open,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

int create_asusproc_gyroscopempusensor_status_entry(void){
	gyroscopempusensor_entry = proc_create("gyroscope_status", S_IWUGO| S_IRUGO, NULL,&gyroscopempusensor_proc_fops);
	if (!gyroscopempusensor_entry)
		 return -ENOMEM;
	return 0;
}
//<asus-annacheng20150129><<<<<<<<<<<<<<<<<+
	...
	const struct i2c_device_id *id)
{
	...
	printk("[mpu6515] probe ++++\n");
#if 0
//<asus_zx20150328>+>>
	//dev_err(&client->dev, "mpu6515 probe, zxtest select pinctrl state\n");
	result = pinctrl_select_state( devm_pinctrl_get(&client->dev), pinctrl_lookup_state(devm_pinctrl_get(&client->dev), "mpu_default"));
		if (result) {
			dev_err(&client->dev, "Can't select pinctrl state\n");
		}

	printk("[mpu6515] enter inv_mpu_probe addr=%d irq=%d id->name=%s\n", client->addr, client->irq, id->name);
#endif
...
//<asus_zx20150328>+<<
	...
	st->client->irq = gpio_to_irq(st->client->irq);
#ifdef CONFIG_DTS_INV_MPU_IIO
	...
	//st->plat_data = gyro_platform_data;

	st->plat_data = ZD550KL_gyro_platform_data;
#endif
...
//<asus-annacheng20150129>>>>>>>>>>>>>>>>>>>>>>>+
	...
	if(create_asusproc_mpusensor_status_entry())
		printk("[%s] : ERROR to create mpusensor proc entry\n",__func__);
	if(create_asusproc_gyroscopempusensor_status_entry())
		printk("[%s] : ERROR to create cpmpassmpusensor proc entry\n",__func__);
//<asus-annacheng20150129><<<<<<<<<<<<<<<<<+
	return 0;
out_remove_wakelock:
...
static int inv_mpu_remove(struct i2c_client *client)
{
	...
	if(gyroscopempusensor_entry)
		remove_proc_entry("gyroscope_status", NULL);
	//<asus-annacheng20150129><<<<<<<<<<<<<<<<<+
#ifdef INV_KERNEL_3_10
	iio_device_free(indio_dev);
#else
