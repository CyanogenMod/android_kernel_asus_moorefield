...
//<ASUS_Proximity->
			...
			printk("synaptics x=%d , y=%d, touch_count=%d\n", x, y, touch_count);
			if (rmi4_data->hw_if->board_data->x_flip)
				x = rmi4_data->sensor_max_x - x;
			if (rmi4_data->hw_if->board_data->y_flip)
				y = rmi4_data->sensor_max_y - y;			
	...
	int writelen, char *readbuf, int readlen)
{
	...
	retry=0;
	printk("[cap] %s: i2c read error. error code = %d\n", __func__, ret);	
	return ret;
}

int cap_i2c_write(struct synaptics_rmi4_data *rmi4_data, char *writebuf, int writelen)
{
	int ret;
	int retry = 0, retrycount = 5;
	struct i2c_client *i2c = to_i2c_client(rmi4_data->pdev->dev.parent);
	...
	retry=0;
	printk("[cap] %s: i2c write error. error code = %d\n", __func__, ret);	
	return ret;
}
...
static void cap_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	int cap_read_addr = 0x00;
	cap_i2c_Read(rmi4_data, &cap_read_addr, 1, &buf_val, 2);
	
	if(buf_val[0] == 1) {	
		printk("[cap] %s : back key buf_val=%d\n", __func__, buf_val[0]);
		input_report_key(rmi4_data->input_dev, KEY_BACK, 1);
		input_sync(rmi4_data->input_dev);
		btn_back=1;
	}else if(buf_val[0] == 2) {		
		printk("[cap] %s : menu key buf_val=%d\n", __func__, buf_val[0]);
		input_report_key(rmi4_data->input_dev, KEY_MENU, 1);
		input_sync(rmi4_data->input_dev);
		btn_menu=1;
	}else {
		if(btn_back == 1){
			printk("[cap] cap sensor buf[0]=%d\n", buf_val[0]);
...
static irqreturn_t cap_sensor_interrupt(int irq, void *data)
{
	...
	{
		disable_irq_nosync(CAP_INT_PIN);
	}
...
//cap_sensor+
static void cap_sensor_reg_init(struct synaptics_rmi4_data *rmi4_data)
{
	int i,ret;
	unsigned char reg_addr;	
	unsigned char buf_val[30]={0};
	printk("[cap] cap sensor start\n");	
	//Sensor status
	reg_addr = 0x00;	
	ret = cap_i2c_Read(rmi4_data, &reg_addr, 1, &buf_val, 1);
	if(ret < 0){
		printk("[cap] read reg_addr=%d, ret =%d\n", reg_addr, ret);
	}
	printk("[cap] cap sensor reg_addr=%x, buf[0]=%x \n", reg_addr, buf_val[0]);
	...
	if(ret < 0){
		printk("[cap] write reg_addr=%d, ret =%d\n", reg_addr, ret);
	}
...
static int __devinit synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	retval = request_threaded_irq(gpio_to_irq(CAP_INT_PIN), NULL, cap_sensor_interrupt,
