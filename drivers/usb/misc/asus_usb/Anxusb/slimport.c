...
#define USB_INFO(...)	printk("[USB][ANALOG_USB_Driver] " __VA_ARGS__);
...
******************************************************************************/
int analog_usb_drv_support = 0;
static int __init analog_usb_setup(char *str)
{
	if (strncmp(str, "analogusb", 9) == 0) {
			analog_usb_drv_support = 1;
	}
	return 1;
}
__setup("asus.dbg.typec=", analog_usb_setup);

static int __init anx7816_init(void)
{

    if (!analog_usb_drv_support) {
        USB_INFO("%s skip this driver...\n", __func__);
        return 0;
    }
