...
#define USB_CORE_INFO(...)	printk("[USB][CORE] " __VA_ARGS__);
...
FSC_U16 core_get_advertised_current(void)
{
    ...
    else if(sourceOrSink == Source)
    {
    	USB_CORE_INFO("now : source\n");
...
export(core_get_advertised_current);
