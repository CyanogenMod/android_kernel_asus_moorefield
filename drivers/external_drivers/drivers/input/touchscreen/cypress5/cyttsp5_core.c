		...
		bool create)
{
	...
	} else if (cd->num_hid_reports >= CY_HID_MAX_REPORTS) {
		dev_err(cd->dev, "%s: requested too many reports: %d >= %d\n",
			__func__,
			cd->num_hid_reports, CY_HID_MAX_REPORTS);
	}
...
static void cyttsp5_si_get_sensing_conf_data(struct cyttsp5_core_data *cd)
{
	...
	si->quirks = 0;
	/* Earlier firmware reports wrong touchpad extents */
	if (scd->res_x == CY_FW_REPORTED_RES_X) {
		scd->res_x = CY_CORRECTED_RES_X;
		si->quirks |= CY_RES_X_QUIRK;
	}
	if (scd->res_y == CY_FW_REPORTED_RES_Y) {
		scd->res_y = CY_CORRECTED_RES_Y;
		si->quirks |= CY_RES_Y_QUIRK;
	}
...
static int move_gesture_data(struct cyttsp5_core_data *cd,
	struct cyttsp5_sysinfo *si)
{
	int size;
	...
	if (size > 0x26) {
		si->gesture_id = cd->input_buf[0x25];
		si->gesture_data = cd->input_buf[0x26];
	} else {
		si->gesture_id = 0;
	}
}
...
/* Protects cyttsp5_core_data_instance and cyttsp5_sleep_state */
DEFINE_MUTEX(cyttsp5_sleep_lock);
static enum { CYTTSP5_ASLEEP, CYTTSP5_AWAKE } cyttsp5_sleep_state;
static struct cyttsp5_core_data *cyttsp5_core_data_instance;

/*
 * Must be called under cyttsp5_sleep_lock
 */
static int cyttsp5_core_sleep_devices(void)
{
	int rc;

	if (cyttsp5_core_data_instance == NULL)
		return 0;

	rc = cyttsp5_core_sleep(cyttsp5_core_data_instance);
	if (rc < 0) {
		printk(KERN_ERR "%s: Error on sleep\n", __func__);
		return -EAGAIN;
	}
 ...
 * Must be called under cyttsp5_sleep_lock
 */
static int cyttsp5_core_wake_devices(void)
{
	int rc;
	...
	rc = cyttsp5_core_wake(cyttsp5_core_data_instance);
	if (rc < 0) {
		printk(KERN_ERR "%s: Error on wake\n", __func__);
		return -EAGAIN;
	}
...
extern int cyttsp5_mt_sleep_devices(void);
extern int cyttsp5_mt_wake_devices(void);
extern int cyttsp5_device_access_sleep_devices(void);
extern int cyttsp5_device_access_wake_devices(void);

int cyttsp5_sleep_devices(void)
{
	int rc;
	char comm[TASK_COMM_LEN];

	printk(KERN_INFO "%s: touchpad deep sleep by pid %d (%s)\n",
	       __func__, current->pid, get_task_comm(comm, current));

	mutex_lock(&cyttsp5_sleep_lock);
	/* idempotency */
	if (cyttsp5_sleep_state != CYTTSP5_AWAKE) {
		rc = -EALREADY;
		goto exit;
	}

	/* The order of these has been considered */
	rc = cyttsp5_device_access_sleep_devices();
	if (rc < 0)
		goto exit;
	rc = cyttsp5_core_sleep_devices();
	if (rc < 0)
		goto device_access_wake;
	rc = cyttsp5_mt_sleep_devices();
	if (rc < 0)
		goto core_wake;

	cyttsp5_sleep_state = CYTTSP5_ASLEEP;
	goto exit;

core_wake:
	rc = cyttsp5_core_wake_devices();
	WARN_ON(rc < 0);
device_access_wake:
	rc = cyttsp5_device_access_wake_devices();
	WARN_ON(rc < 0);
exit:
	if (rc < 0)
		printk(KERN_WARNING "%s: touchpad deep sleep failed (%d)\n",
		       __func__, rc);
...
int cyttsp5_wake_devices(void)
{
	int rc;
	char comm[TASK_COMM_LEN];

	printk(KERN_INFO "%s: touchpad woken by pid %d (%s)\n",
	       __func__, current->pid, get_task_comm(comm, current));

	mutex_lock(&cyttsp5_sleep_lock);
	/* idempotency */
	if (cyttsp5_sleep_state != CYTTSP5_ASLEEP) {
		rc = -EALREADY;
		goto exit;
	}
	rc = cyttsp5_mt_wake_devices();
	if (rc < 0)
		goto exit;
	rc = cyttsp5_core_wake_devices();
	if (rc < 0)
	...
	cyttsp5_sleep_state = CYTTSP5_AWAKE;
exit:
	if (rc < 0)
		printk(KERN_WARNING "%s: touchpad wake failed (%d)\n",
		       __func__, rc);
...
static ssize_t cyttsp5_sleep_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int rc = cyttsp5_sleep_devices();
	if (rc < 0)
		return rc;
	return size;
}

static ssize_t cyttsp5_wake_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int rc = cyttsp5_wake_devices();
	if (rc < 0)
		return rc;
	return size;
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(wake, S_IWUSR, NULL, cyttsp5_wake_store),
};
...
static int cyttsp5_core_probe(struct cyttsp5_core *core)
{
	...
	if (rc < 0) {
		dev_err(cd->dev, "%s: HW Init fail r=%d. Attempt hard reset\n", __func__, rc);
	...
	WARN_ON(cyttsp5_core_data_instance != NULL);
	if (cyttsp5_core_data_instance == NULL) {
		cyttsp5_core_data_instance = cd;
		cyttsp5_sleep_state = CYTTSP5_AWAKE;
	}
	mutex_unlock(&cyttsp5_sleep_lock);
...
static int cyttsp5_core_release(struct cyttsp5_core *core)
{
	...
	mutex_lock(&cyttsp5_sleep_lock);
	cyttsp5_core_data_instance = NULL;
	mutex_unlock(&cyttsp5_sleep_lock);
...
static struct cyttsp5_core_driver cyttsp5_core_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		/* Touchpad is a wake device, it can't sleep */
		.pm = NULL,
	},
};
