	...
	const ia_css_process_group_cmd_t		cmd)
{
	...
	case IA_CSS_PROCESS_GROUP_CMD_START:
		...
		verifexit(ia_css_bxt_pss_process_group_load_addtional(process_group) == 0, EINVAL);

		process_group->state = IA_CSS_PROCESS_GROUP_STARTED;

		verifexit(ia_css_process_group_run(process_group) == 0, EINVAL);

		break;
	case IA_CSS_PROCESS_GROUP_CMD_RUN:
		verifexit(state == IA_CSS_PROCESS_GROUP_STARTED, EINVAL);
		verifexit(ia_css_bxt_pss_process_group_run(process_group) == 0, EINVAL);
	...
	case IA_CSS_PROCESS_GROUP_CMD_STOP:
		...
		verifexit(ia_css_bxt_pss_process_group_stop(process_group) == 0, EINVAL);
