...
void ia_css_devproxy_service_wba_init_ref(
...
void ia_css_devproxy_service_wba_route_out(
	unsigned int service_pid, /* [in] the service to address */
	unsigned int channel_num, /* [in] the channel to use */
	uint32_t ack_addr,
	uint32_t ack_token
	)
{
	vied_subsystem_address_t addr = get_service_addr(service_pid, channel_num);
	store_data_to_proxy(
		addr + offsetof(struct ia_css_devproxy_service_wba_state_type, ack_addr),
		&ack_addr,
		sizeof(ack_addr)) VOLATILE;
	store_data_to_proxy(
		addr + offsetof(struct ia_css_devproxy_service_wba_state_type, ack_token),
		&ack_token,
		sizeof(ack_token)) VOLATILE;
}

void ia_css_devproxy_service_wba_config(
       unsigned int service_pid, /* [in] the service to address */
       unsigned int channel_num, /* [in] the channel to use */
       uint32_t num_lines)
{
	vied_subsystem_address_t addr = get_service_addr(service_pid, channel_num);
	store_data_to_proxy(
		addr + offsetof(struct ia_css_devproxy_service_wba_state_type, num_lines),
		&num_lines,
