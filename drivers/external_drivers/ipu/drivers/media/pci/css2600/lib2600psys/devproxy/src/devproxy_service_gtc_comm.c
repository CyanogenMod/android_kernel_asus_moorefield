	...
	)
{
	vied_subsystem_address_t addr = get_service_addr(service_pid, channel_num);
	store_data_to_proxy(
		addr + offsetof(struct ia_css_devproxy_service_gtc_state_type, ack_addr),
		&ack_addr,
		sizeof(ack_addr)) VOLATILE;
	store_data_to_proxy(
		addr + offsetof(struct ia_css_devproxy_service_gtc_state_type, ack_token),
		&ack_token,
       ...
       uint32_t num_lines)
{
	vied_subsystem_address_t addr = get_service_addr(service_pid, channel_num);
	store_data_to_proxy(
		addr + offsetof(struct ia_css_devproxy_service_gtc_state_type, num_lines),
		&num_lines,
