...
#define _vied_align(address, bytes) ((((unsigned long long)(address)) + ((bytes)-1)) & ~((bytes)-1))

#define _hrt_vied_set_icache_master_icache(cell, segment, offset) \
{\
  unsigned int addr = aligned_ddr_addr;\
...
{ \
  unsigned int da = (unsigned int)ddr_addr; \
  unsigned long long hda = (unsigned long long)host_ddr_addr; \
  da = da; \
  hda = hda; \
}
...
#define vied_alloc_program(prog) \
  SHARED_MEMORY_FOR_CRUN_NOT_DEFINED
#else
#define vied_alloc_program(prog) \
