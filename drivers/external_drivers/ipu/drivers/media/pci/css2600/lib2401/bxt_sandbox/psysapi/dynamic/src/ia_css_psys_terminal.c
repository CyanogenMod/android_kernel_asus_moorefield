	...
	const ia_css_terminal_param_t			*terminal_param)
{
	...
	if (ia_css_is_terminal_manifest_data_terminal(manifest) == true) {
		...
		if (section_count != 0) {
			...
			size_t offset_size = section_count * sizeof(uint32_t);
			ia_css_param_terminal_t *pterminal = (ia_css_param_terminal_t *)terminal;

			/* allocate memory for parameter_descriptor(that holds info for all sections)*/
			tmp_size = sizeof(ia_css_kernel_param_descriptor_t);
			...
			/* allocate memory to hold individual offsets  */
			pterminal->param_descriptor->section_offsets = (uint32_t *)
						ia_css_cpu_mem_alloc(offset_size);
			verifexit(pterminal->param_descriptor->section_offsets  != NULL, ENOBUFS);
			ia_css_cpu_mem_set_zero(pterminal->param_descriptor->section_offsets, offset_size);

			pterminal->param_descriptor->size = tmp_size + offset_size; /* size of structure + size of offsets*/
			pterminal->param_descriptor->section_count = section_count;

			/*  assign individual offsets using param terminal's manifest.*/
			for(i = 0 ; i < section_count; i++){
				ia_css_parameter_manifest_t *param_manifest =
						ia_css_param_terminal_manifest_get_parameter_manifest(
							(ia_css_param_terminal_manifest_t *)manifest,
							i);
				verifexit(param_manifest != NULL, EINVAL);
				*(pterminal->param_descriptor->section_offsets++) =
						param_manifest->mem_offset;
			}
			size += tmp_size;
		}
	} else {
