	...
	struct ia_css_cas_binary_descr *descr)
{
	...
	/* For CTS test -- testVideSnapshot, 640x480 recording in ov5670 may failed*/
	if(hor_ds_factor == ver_ds_factor){
		printk("OK for assert condition : (hor_ds_factor == ver_ds_factor)\n");
	}else{
		printk("CTS workaround: DisFailed assert(hor_ds_factor == ver_ds_factor) (%d,%d)/ W(%d,%d) , H(%d,%d)\n",hor_ds_factor,ver_ds_factor,cas_scaler_in_info->res.width , cas_scaler_out_info->res.width , cas_scaler_in_info->res.height, cas_scaler_out_info->res.height);
		printk("CTS workaround: FATAL error , please fixed me!!\n");
	}
