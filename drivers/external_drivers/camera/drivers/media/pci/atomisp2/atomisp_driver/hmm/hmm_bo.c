			     ...
			     unsigned long shared_fd)
{
	...
	bo->bdev->iclient = (struct ion_client *) ion_file->private_data;
	if (unlikely(!bo->bdev->iclient)) {
		dev_err(atomisp_dev, "invalid ion file struct for iclient.\n");
		ret = -EINVAL;
		goto error;
	}

	bo->ihandle = ion_import_dma_buf(bo->bdev->iclient, ion_shared_fd);
	...
	for_each_sg(sg_tbl->sgl, sl, sg_tbl->nents, i) {
...
#else
		if (unlikely(sl == NULL)) {
			dev_err(atomisp_dev, "Null scatterlist pointer.\n");
			dump_stack();
			ret = -EINVAL;
			goto error_unmap;
		}
		...
		if (unlikely(page == NULL)) {
			dev_err(atomisp_dev, "Null pointer Table of SG Pages.\n");
			dump_stack();
			ret = -EINVAL;
			goto error_unmap;
		}
