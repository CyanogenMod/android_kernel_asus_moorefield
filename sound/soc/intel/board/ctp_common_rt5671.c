			...
			struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		/*Set GPIO O(L) to default => Low:disable; High:enable */
		gpio_direction_output(spk_amp, 1);
		pr_info("SPK_AMP_EN value = %x\n", gpio_get_value(spk_amp));
	} else {
		/*Set GPIO O(L) to default => Low:disable; High:enable */
		gpio_direction_output(spk_amp, 0);
		pr_info("SPK_AMP_EN value = %x\n", gpio_get_value(spk_amp));
...
static void local_wake_lock(struct ctp_mc_private *ctx)
{
#ifdef CONFIG_HAS_WAKELOCK
	wake_lock(ctx->jack_wake_lock);
#endif
}

static void local_wake_unlock(struct ctp_mc_private *ctx)
{
#ifdef CONFIG_HAS_WAKELOCK
...
static void local_wake_lock_destroy(struct ctp_mc_private *ctx)
{
#ifdef CONFIG_HAS_WAKELOCK
	wake_unlock(ctx->jack_wake_lock);
...
void cancel_all_work(struct ctp_mc_private *ctx)
{
	...
	/* remove the wakelock that was possibly taken before scheduling a work */
	local_wake_unlock(ctx);
}
...
int ctp_soc_jack_gpio_detect(void)
{
	...
	/* Take wakelock prior to scheduling the work. Since timers
	 * are deactivated in S3, this will prevent system freeze
	 * during e.g MICBIAS debounce sleep or gpio debounce time.
	 * Jack detection is happening rarely so this doesn't
...
void headset_insert_poll(struct work_struct *work)
{
	...
	if (!enable) {
		...
		/* Remove the lock taken prior to scheduling the work */
		local_wake_unlock(ctx);
		return;
	}
...
int snd_ctp_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	spk_amp = get_gpio_by_name("SPK_AMP_EN");
	if (spk_amp > 0) {
		pr_info("Get SPK_AMP_EN name!\n");
		ret = gpio_request_one(spk_amp, GPIOF_DIR_OUT, "SPK_AMP_EN");
		if (ret)
			pr_err("gpio_request SPK_AMP_EN failed!\n");
	} else
		pr_err("get_gpio SPK_AMP_EN failed!\n");
