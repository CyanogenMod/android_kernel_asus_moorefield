...
#define VERSION "0.1.1 alsa 1.0.25"
...
static struct rt5647_init_reg init_list[] = {
	...
	{ RT5647_HPOMIXL_CTRL	, 0x001e },/* DAC1/2 -> HPOVOL */
	{ RT5647_HPOMIXR_CTRL	, 0x001e },/* DAC1/2 -> HPOVOL */
	...
	{ RT5647_SPK_L_MIXER	, 0x003c },/* DAC1/2 -> SPKVOL */
	{ RT5647_SPK_R_MIXER	, 0x003c },/* DAC1/2 -> SPKVOL */
	{ RT5647_SPO_MIXER	, 0xc806 },/* SPKVOL -> SPO */
#endif
...
int rt5647_check_irq_event(struct snd_soc_codec *codec)
{
	...
	//pr_info(" rt5647_check_irq_event : val = 0x%x rt5647->jack_type=0x%x\n", val, rt5647->jack_type);
	printk(KERN_INFO "%s: val = 0x%x rt5647->jack_type=0x%x\n",__func__, val, rt5647->jack_type);
	switch (val) {
	case 0x20: /* No plug */
...
static const struct snd_kcontrol_new rt5647_snd_controls[] = {
	...
	SOC_SINGLE_TLV("OUTMIXR BST2 Gain", RT5647_OUTMIXR_CTRL1,
			7, 3, 1, dac_gain_tlv),
	/* OUT Gain */
	SOC_SINGLE_TLV("HPO MIX Gain", RT5647_GEN_CTRL2, 6, 1, 1, out_gain_tlv),
	SOC_SINGLE_TLV("Speaker Output Gain", RT5647_SPO_CLSD_RATIO,
		       0, 7, 0, speaker_gain_tlv),
	SOC_SINGLE_TLV("LOUT MIX Gain", RT5647_LOUT_MIXER,
			RT5647_G_LOUTMIX_SFT, 1, 1, out_gain_tlv),
	SOC_SINGLE_TLV("MONOAmp MIX Gain", RT5647_MONO_MIXER2,
			RT5647_G_MONOMIX_SFT, 1, 1, out_gain_tlv),
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_update_bits(codec, RT5647_PWR_VOL,
			...
			RT5647_PWR_HV_L | RT5647_PWR_HV_R);
		snd_soc_update_bits(codec, RT5647_PWR_MIXER,
			RT5647_PWR_HM_R | RT5647_PWR_HM_L,
			RT5647_PWR_HM_R | RT5647_PWR_HM_L);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		queue_delayed_work(spk_unmute_wq, &spk_unmute_work, msecs_to_jiffies(spk_unmute_delay_time));
		break;

	case SND_SOC_DAPM_PRE_PMD:
		cancel_delayed_work_sync(&spk_unmute_work);
		spk_unmute_delay_time = 0;
		snd_soc_update_bits(codec, RT5647_GPIO_CTRL4, 0x0002, 0x0000);
...
static int rt5647_inl_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMD:
		val = snd_soc_read(w->codec, RT5647_HPOMIXL_CTRL) & RT5647_M_IN_HV;
...
static int rt5647_inr_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	static unsigned int val;
	...
	case SND_SOC_DAPM_PRE_PMU:
	case SND_SOC_DAPM_PRE_PMD:
		val = snd_soc_read(w->codec, RT5647_HPOMIXR_CTRL) & RT5647_M_IN_HV;
...
static int rt5647_dac_sto_filter_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	static unsigned int val;
	...
	case SND_SOC_DAPM_PRE_PMU:
	case SND_SOC_DAPM_PRE_PMD:
		val = snd_soc_read(w->codec, RT5647_STO_DAC_MIXER);
		snd_soc_write(w->codec, RT5647_STO_DAC_MIXER, 0x5656);
		break;
	case SND_SOC_DAPM_POST_PMU:
	case SND_SOC_DAPM_POST_PMD:
		msleep(10);
		snd_soc_write(w->codec, RT5647_STO_DAC_MIXER, val);
		break;
	default:
		return 0;
	}
...
static const struct snd_soc_dapm_widget rt5647_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA_E("INL VOL", RT5647_PWR_VOL,
		...
		rt5647_inl_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD |
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("INR VOL", RT5647_PWR_VOL,
		RT5647_PWR_IN_R_BIT, 0, NULL, 0,
		rt5647_inr_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD |
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	/* IN Mux */
	...
	SND_SOC_DAPM_SUPPLY_S("dac stereo1 filter", 1, RT5647_PWR_DIG2,
		RT5647_PWR_DAC_S1F_BIT, 0, rt5647_dac_sto_filter_event,
	...
	SND_SOC_DAPM_PGA("OUTVOLL", RT5647_PWR_VOL,
        RT5647_PWR_OV_L_BIT, 0, NULL, 0),
	SND_SOC_DAPM_PGA("OUTVOLR", RT5647_PWR_VOL,
        RT5647_PWR_OV_R_BIT, 0, NULL, 0),
...
static const struct snd_soc_dapm_route rt5647_dapm_routes[] = {
	...
	{"INR VOL", NULL, "DAC L1 Power"},
	{"INR VOL", NULL, "DAC R1 Power"},
	{"INL VOL", NULL, "PLL1"},
	{"INR VOL", NULL, "PLL1"},

/*
	{"INL VOL", NULL, "HPOVOL MIXL"},
	{"INL VOL", NULL, "HPOVOL MIXR"},
	{"INR VOL", NULL, "HPOVOL MIXL"},
	{"INR VOL", NULL, "HPOVOL MIXR"},
*/
	...
	{ "OUTVOLL", NULL, "OUT MIXL"},
	{ "OUTVOLR", NULL, "OUT MIXR"},
	{ "LOUT MIX", "DAC L1 Switch", "DAC L1" },
	{ "LOUT MIX", "DAC R1 Switch", "DAC R1" },
	{ "LOUT MIX", "OUTMIX L Switch", "OUTVOLL" },
	{ "LOUT MIX", "OUTMIX R Switch", "OUTVOLR" },
...
static DEVICE_ATTR(index_reg, 0664, rt5647_index_show, rt5647_index_store);
...
static DEVICE_ATTR(codec_reg, 0664, rt5647_codec_show, rt5647_codec_store);
...
static void jd_check_handler(struct work_struct *work)
{
	...
	if ((val == 0x20 && pr_check == 0) || (mrfld_hs_enable == 0 && pr_check == 1)) {
		...
		snd_soc_jack_report(rt5647_jack, 0, SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_BTN_2 | SND_JACK_BTN_3);
		rt5647_headset_detect(codec, 0);
		msleep(50);
		/*snd_soc_jack_report(jack, 0, SND_JACK_HEADSET | SND_JACK_BTN_0);*/
		snd_soc_jack_report(rt5647_jack, 0, SND_JACK_HEADSET | SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_BTN_2 | SND_JACK_BTN_3);
		del_timer(&jd_check_timer);
	}
}
...
struct proc_dir_entry * audio_entry = NULL;
static int audio_proc_show(struct seq_file *m, void *v) {
...
static int  audio_proc_open(struct inode *inode, struct  file *file) {

  return single_open(file, audio_proc_show, NULL);

}

static const struct file_operations  audio_proc_fops = {

  .owner = THIS_MODULE,

  .open =  audio_proc_open,
...
int create_asusproc_audio_status_entry( void )

{
   audio_entry = proc_create("asus_codec_status", S_IWUGO| S_IRUGO, NULL,& audio_proc_fops);
...
#ifdef RT5647_VER_B /*Set micbias 1 1.8V*/
	snd_soc_update_bits(codec, RT5647_MICBIAS,
				RT5647_MIC1_BS_MASK, RT5647_MIC1_BS_75AV);
#endif
...
#ifdef CONFIG_ASUS_FACTORY_MODE
	create_asusproc_audio_status_entry();
#endif
		    ...
		    const struct i2c_device_id *id)
{
	...
	if (NULL == rt5647) {
		printk(KERN_INFO "%s: allocate memory fail", __func__);
		return -ENOMEM;
	}
