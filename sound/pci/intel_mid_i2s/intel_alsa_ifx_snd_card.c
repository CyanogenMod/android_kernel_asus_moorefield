/*
 *   intel_alsa_ifx_snd_card.c - Intel Sound card driver for IFX
 *
 ...
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * ALSA driver for IFX sound card
 ...
 * The ALSA IFX Driver interfaces with the Intel MID SSP Driver to
...
#include "intel_alsa_ifx_snd_card.h"

MODULE_AUTHOR("Selma Bensaid <selma.bensaid@intel.com>");
MODULE_DESCRIPTION("Intel ALSA IFX Sound Card Driver");
MODULE_LICENSE("GPL v2");
MODULE_SUPPORTED_DEVICE("{Intel,INTEL_ALSA_IFX_SND_CARD");
MODULE_VERSION(INTEL_ALSA_SSP_DRIVER_VERSION);

#define intel_alsa_ifx_stream_info intel_alsa_ssp_stream_info

static struct intel_alsa_ifx_card_info *p_alsa_ifx_snd_card;

/* Index of sound card will be allocated by the kernel */
static int v_intel_alsa_ifx_card_index = SNDRV_DEFAULT_IDX1;
static char *p_intel_alsa_ifx_card_id = SNDRV_DEFAULT_STR1;

module_param(v_intel_alsa_ifx_card_index, int, 0444);
MODULE_PARM_DESC(v_intel_alsa_ifx_card_index,
		"Index value for ALSA IFX PCM soundcard.");
module_param(p_intel_alsa_ifx_card_id, charp, 0444);
MODULE_PARM_DESC(p_intel_alsa_ifx_card_id,
		"ID string ALSA IFX PCM soundcard.");


/*
 * snd_i2s_alsa_ifx_open- to set runtime parameters during stream start
 ...
 *
 */
int snd_i2s_alsa_ifx_open(struct snd_pcm_substream *substream)
{
	...
	WARN(!substream->pcm, "ALSA_IFX: ERROR NULL substream->pcm\n");
	if (!substream->pcm)
		return -EINVAL;

	device = substream->pcm->device;

	pr_debug("ALSA_IFX: FCT %s snd pcm device %d\n",
			__func__,
			substream->pcm->device);

	/* set the runtime hw parameter with local snd_pcm_hardware struct */
	str_runtime->hw = *(s_ifx_dev_info[device].stream_hw_param);
	...
	for (i = 0; i < device; i++)
		str_info->stream_index += s_ifx_dev_info[i].nb_capt_stream +
		  s_ifx_dev_info[i].nb_play_stream;

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		str_info->stream_index++;

	INIT_WORK(&str_info->ssp_ws, intel_alsa_ifx_transfer_data);

	/* Initialize SSP0 driver */
	/* Store the Stream information */
	str_runtime->private_data = str_info;

	str_info->substream = substream;

	ret_value = intel_alsa_ifx_control(INTEL_ALSA_SSP_CTRL_SND_OPEN,
			str_info);
...
/*
 * snd_i2s_alsa_ifx_close- to free parameters when stream is stopped
 ...
 *
 */
int snd_i2s_alsa_ifx_close(struct snd_pcm_substream *substream)
{
	struct intel_alsa_ifx_stream_info *str_info;
	int ret_val = 0;
	...
	if (str_info) {
		...
		WARN(1, "ALSA_IFX: ERROR p_stream_info is NULL\n");
		return -EINVAL;
	}
	pr_debug("ALSA IFX CLOSE Stream Direction = %d\n",
			substream->stream);
...
/**
 * snd_i2s_alsa_ifx_hw_params - Allocate memory for Ring Buffer according
 * to hw_params.
 ...
 *
 */
int snd_i2s_alsa_ifx_hw_params(struct snd_pcm_substream *substream,
...
/**
 * snd_i2s_alsa_ifx_hw_free - Called to release ressources allocated by
 * snd_i2s_alsa_ifx_hw_params
 * It's called in a non-atomic context
 *
 * * Input parameters
 *		@substream:substream for which the stream function is called
 ...
 *
 */
int snd_i2s_alsa_ifx_hw_free(struct snd_pcm_substream *substream)
{
	WARN(!substream, "ALSA_IFX: ERROR NULL substream\n");
	if (!substream)
		return -EINVAL;
...
/*
 * snd_i2s_alsa_ifx_pcm_pointer- to send the current buffer pointer
 * processed by HW
 ...
 */
snd_pcm_uframes_t
	snd_i2s_alsa_ifx_pcm_pointer(struct snd_pcm_substream *substream)
{
	struct intel_alsa_ifx_stream_info *str_info;
	...
	WARN(!str_info, "ALSA_IFX: ERROR NULL str_info\n");
	if (!str_info)
		return -EINVAL;
	...
	pr_debug("ALSA_IFX: Frame bits:: %d period_size :: %d periods :: %d\n",
	     (int) substream->runtime->frame_bits,
	     (int) substream->runtime->period_size,
	     (int) substream->runtime->periods);

	pr_debug("ALSA_IFX: snd_i2s_alsa_ifx_pcm_pointer returns %ld\n",
			pcm_pointer);
...
/*
 * snd_i2s_alsa_ifx_pcm_prepare- internal preparation before starting a stream
 ...
 *		NA
 */
int snd_i2s_alsa_ifx_pcm_prepare(struct snd_pcm_substream *substream)
{
	return 0;
}
...
/*
 * intel_alsa_ifx_create_pcm_device : to setup pcm for the card
 *
 * Input parameters
 *		@card : pointer to the sound card structure
 *		@p_alsa_ifx_snd_card : pointer to internal context
 *
 * Output parameters
 *		@ret_val : status, 0 ==> OK
 *
 */
static int intel_alsa_ifx_create_pcm_device(struct snd_card *card,
			struct intel_alsa_ifx_card_info *p_alsa_ifx_snd_card)
{
	struct snd_pcm *pl_pcm;
	int i, ret_val = 0;

	WARN(!card, "ALSA_IFX: ERROR NULL card\n");
	if (!card)
		return -EINVAL;

	WARN(!p_alsa_ifx_snd_card, "ALSA_IFX: ERROR NULL alsa_ifx_snd_card\n");
	if (!p_alsa_ifx_snd_card)
		return -EINVAL;

	/*
	 * The alsa_ifx driver handles provide 2 PCM devices :
	 ...
	 */
	for (i = 0; i < INTEL_ALSA_IFX_SND_CARD_MAX_DEVICES; i++) {

		ret_val = snd_pcm_new(card, s_ifx_dev_info[i].dev_name, i,
				s_ifx_dev_info[i].nb_play_stream,
				s_ifx_dev_info[i].nb_capt_stream,
				&pl_pcm);
		...
		snd_pcm_set_ops(pl_pcm, SNDRV_PCM_STREAM_PLAYBACK,
					&s_alsa_ifx_playback_ops);
		snd_pcm_set_ops(pl_pcm, SNDRV_PCM_STREAM_CAPTURE,
					&s_alsa_ifx_capture_ops);

		/* setup private data which can be retrieved when required */
		pl_pcm->private_data = p_alsa_ifx_snd_card;
		pl_pcm->info_flags = 0;
		...
		snd_pcm_lib_preallocate_pages_for_all(pl_pcm,
			...
			(GFP_KERNEL),
			s_ifx_dev_info[i].stream_hw_param->buffer_bytes_max,
			s_ifx_dev_info[i].stream_hw_param->buffer_bytes_max);
	}
...
/**
 * intel_alsa_ifx_create_snd_card : function which creates sound card and PCM
 * device
 *
 * Input parameters
 *		NA
 ...
 *
 */
int intel_alsa_ifx_create_snd_card(void)
{
	struct snd_card *card;
	int ret_val;

	/*
	 * Allocate the global structure of the Driver p_alsa_ifx_snd_card
	 */
	p_alsa_ifx_snd_card = kzalloc(sizeof(*p_alsa_ifx_snd_card), GFP_KERNEL);

	if (!p_alsa_ifx_snd_card) {
		WARN(1, "ALSA IFX ERROR: malloc fail\n");
		return -ENOMEM;
	}
	 ...
	 */
	ret_val = snd_card_create(v_intel_alsa_ifx_card_index,
			p_intel_alsa_ifx_card_id, THIS_MODULE, 0, &card);

	if (ret_val) {
		pr_debug("ALSA IFX ERROR: snd_card_create fail\n");
		goto free_allocs;
	}

	p_alsa_ifx_snd_card->card = card;
	p_alsa_ifx_snd_card->card_id = p_intel_alsa_ifx_card_id;
	p_alsa_ifx_snd_card->playback_cnt =
			p_alsa_ifx_snd_card->capture_cnt = 0;
	...
	ret_val = intel_alsa_ifx_create_pcm_device(card, p_alsa_ifx_snd_card);
	if (ret_val) {
		pr_err("ALSA IFX ERROR: failed to allocate p_alsa_ifx_snd_card\n");
		goto free_allocs;
	}

	card->private_data = &p_alsa_ifx_snd_card;

	ret_val = snd_card_register(card);

	if (ret_val) {
		pr_err("ALSA IFX: snd_card_register failed\n");
		goto free_allocs;
	}
...
free_allocs:
	snd_card_free(card);
	kfree(p_alsa_ifx_snd_card);
	WARN(1, "ALSA IFX ERROR: Intel ALSA IFX Card Creation failed\n");
	return ret_val;
} /* intel_alsa_ifx_create_snd_card_ifx */

/**
 * intel_alsa_ifx_remove_snd_card : function which removes the sound card & PCM
 * devices to the IFX driver Interface
 ...
 *		@ret_val : status, 0 ==> OK
 */
int intel_alsa_ifx_remove_snd_card(void)
{
	int ret_val;

	if (p_alsa_ifx_snd_card) {
		pr_debug("ALSA_IFX: p_alsa_ifx_snd_card: 0x%08X\n",
				(u32) p_alsa_ifx_snd_card);
		destroy_workqueue(p_alsa_ifx_snd_card->ssp_wq);
		snd_card_free(p_alsa_ifx_snd_card->card);
		kfree(p_alsa_ifx_snd_card);
		ret_val = 0;
	} else {
		WARN(1, "ALSA_IFX: p_alsa_ifx_snd_card is NULL\n");
		ret_val = -EINVAL;
	}
	return ret_val;

} /* intel_alsa_ifx_remove_snd_card */

static int __init intel_alsa_ifx_init(void)
{
	int status;
	pr_info("Intel ALSA IFX Driver\n Version %s\n",
			INTEL_ALSA_SSP_DRIVER_VERSION);
	status = intel_alsa_ifx_create_snd_card();
	return status;

}

static void __exit intel_alsa_ifx_exit(void)
{
	int status;

	status = intel_alsa_ifx_remove_snd_card();
	pr_info("Intel ALSA IFX Driver Remove with status = %d\n", status);

}

module_init(intel_alsa_ifx_init);
module_exit(intel_alsa_ifx_exit);
