...
//		lnw_gpio_set_alt(GP_CAMERA_0_1P2, LNW_GPIO);
	...
	case PROJ_ID_ZX550ML:
	    switch (Read_HW_ID()) {
		case HW_ID_EVB:
			pr_info("Hardware VERSION = EVB, t4k35 does not support.\n");
			break;
		case HW_ID_SR1:
			pr_info("t4k35 --> HW_ID_SR1\n");
			if (camera_2v8 < 0) {
		...
		case HW_ID_PR:
		case HW_ID_pre_PR:
			pr_info("t4k35 --> HW_ID_PR\n");
			break;
		case HW_ID_MP:
			pr_info("t4k35 --> HW_ID_MP\n");
			break;
		default:
			pr_info("t4k35 --> HW_ID does not define\n");
		break;
	    }
	break;
