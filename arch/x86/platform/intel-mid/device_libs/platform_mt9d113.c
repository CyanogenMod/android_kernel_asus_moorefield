/*
 * platform_mt9d113.c: mt9d113 platform data initilization file
 *
 ...
 */
static int mt9d113_gpio_ctrl(struct v4l2_subdev *sd, int flag)
{
	int ret;
...
static int mt9d113_flisclk_ctrl(struct v4l2_subdev *sd, int flag)
{
...
static int mt9d113_power_ctrl(struct v4l2_subdev *sd, int flag)
{
	...
	if (camera_power_down < 0) {
		ret = camera_sensor_gpio(-1, GP_CAMERA_0_STANDBY,
					 GPIOF_DIR_OUT, 1);
		if (ret < 0) {
			pr_err("%s not available.", GP_CAMERA_0_STANDBY);
			return ret;
		}
...
static int mt9d113_csi_configure(struct v4l2_subdev *sd, int flag)
{
	return camera_sensor_csi(sd, ATOMISP_CAMERA_PORT_PRIMARY, 1,
		ATOMISP_INPUT_FORMAT_YUV422_8, -1, flag);
}

static struct camera_sensor_platform_data mt9d113_sensor_platform_data = {
	.gpio_ctrl	= mt9d113_gpio_ctrl,
	.flisclk_ctrl	= mt9d113_flisclk_ctrl,
	.power_ctrl	= mt9d113_power_ctrl,
	.csi_cfg	= mt9d113_csi_configure,
};
