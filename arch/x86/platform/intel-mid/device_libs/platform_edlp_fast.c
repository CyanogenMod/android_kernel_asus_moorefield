/*
 * platform_edlp_fast.c: hsi EDLP platform data initilization file
 *
...
void edlp_fast_store_modem_ver(const char *modem_name)
{
	int index = 0;
...
void *edlp_fast_platform_data(void *data)
{
	int is_v2, i;
	...
	pr_debug("%s, device name is %s", __func__, hsi_sfi_info->name);
	/* HSI IP v2 ? */
...
void sfi_handle_edlp_fast_dev(struct sfi_device_table_entry *pentry,
							struct devs_id *dev)
{
	...
	if (pentry->type == SFI_DEV_TYPE_HSI) {
		...
		edlp_fast_store_modem_ver(pentry->name);
}
